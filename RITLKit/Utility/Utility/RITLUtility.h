...
#define RITL_IS_IPHONE_12 ((RITL_SCREEN_WIDTH == 390) && (RITL_SCREEN_HEIGHT == 844))
#define RITL_IS_IPHONE_12_Pro_Max ((RITL_SCREEN_WIDTH == 428) && (RITL_SCREEN_HEIGHT == 926))
#define RITL_HAVE_PHONE_HEADER (RITL_IS_IPHONEX || RITL_IS_IPHONEXR || RITL_IS_IPHONEXS || RITL_IS_IPHONEXS_MAX || RITL_IS_IPHONE_12 || RITL_IS_IPHONE_12_Pro_Max)
#define RITL_iPhoneX RITL_HAVE_PHONE_HEADER //为了适配之前的，建议进行判定使用上面方法

//TODO 用于替代判断是否存在刘海屏的方法
static inline BOOL RITL_HAS_PHONE_HEADER() {
    if (@available(iOS 11.0, *)) {
        NSArray <UIWindow *> *windows = UIApplication.sharedApplication.windows;
        if (windows.count > 0) {
            return windows.firstObject.safeAreaInsets.bottom > 0;
        }
        return false;
    } else {
        return  false;
